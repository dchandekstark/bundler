  ...
  module Helpers
    ...
    def self.bang(method)
      define_method("#{method}!") do |*args, **kwargs, &blk|
        send(method, *args, **kwargs, &blk).tap do
          unless last_command.success?
            raise "Invoking #{method}!(#{(args.map(&:inspect) + kwargs.map(&:inspect)).join(", ")}) failed:\n#{last_command.stdboth}"
          end
        end
      end
    end
    ...
    def ruby(ruby, options = {})
      ...
      sys_exec(%(#{Gem.ruby}#{lib_option} -w -e #{ruby.shellescape}), :env => env, :dir => dir)
    end
    bang :ruby
    ...
    def gem_command(command, args = "", dir: bundled_app)
